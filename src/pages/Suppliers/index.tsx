...
import { suppliersApi } from '@/services/finance'
...
export function SuppliersPage() {
  ...
  const { data, isLoading } = useQuery({ queryKey: ['suppliers', page, search], queryFn: () => suppliersApi.list({ page, limit: 10, search }) })

  const save = useMutation({
    mutationFn: (d: FormData) => editing ? suppliersApi.update(editing.id, d) : suppliersApi.create(d),
    onSuccess: () => { qc.invalidateQueries({ queryKey: ['suppliers'] }); setModalOpen(false); setEditing(null) },
  })
  ...
  const openEdit = (s: Supplier) => { reset(s as FormData); setEditing(s); setModalOpen(true) }

  const columns = [
    { key: 'name', label: 'Fornecedor' },
    { key: 'type', label: 'Tipo', render: (r: Supplier) => r.type === 'pj' ? 'PJ' : 'PF' },
    { key: 'document', label: 'CNPJ/CPF', render: (r: Supplier) => r.document || '-' },
    { key: 'email', label: 'E-mail', render: (r: Supplier) => r.email || '-' },
    { key: 'phone', label: 'Telefone', render: (r: Supplier) => r.phone || '-' },
    { key: 'status', label: 'Status', render: (r: Supplier) => <StatusBadge status={r.status} /> },
    {
      key: 'actions', label: '', width: '100px', render: (r: Supplier) => (
        <Flex gap={1}>
  ...
  return (
    <>
      <PageHeader title="Fornecedores" description="Gerencie os fornecedores da empresa" actions={<Button colorPalette="blue" size="sm" onClick={openCreate}><Plus size={16} />Novo Fornecedor</Button>} />
      <DataTable columns={columns} data={data?.data ?? []} isLoading={isLoading} pagination={data?.pagination} onPageChange={setPage} search={search} onSearch={(v) => { setSearch(v); setPage(1) }} searchPlaceholder="Buscar fornecedor..." />

      <Dialog.Root open={modalOpen} onOpenChange={({ open: o }) => { if (!o) { setModalOpen(false); setEditing(null) } }}>
        <Dialog.Backdrop />
        <Dialog.Positioner>
          <Dialog.Content maxW="500px">
            <Dialog.Header><Dialog.Title>{editing ? 'Editar' : 'Novo'} Fornecedor</Dialog.Title></Dialog.Header>
            <Dialog.Body>
              <form id="supplier-form" onSubmit={handleSubmit((d) => save.mutate(d))}>
            ...
            <Dialog.Footer gap={3}>
              ...
              <Button form="supplier-form" type="submit" colorPalette="blue" loading={save.isPending}>Salvar</Button>
            </Dialog.Footer>
          </Dialog.Content>
        </Dialog.Positioner>
      </Dialog.Root>

      <ConfirmDialog open={!!deleteTarget} title="Excluir Fornecedor" description={`Deseja excluir "${deleteTarget?.name}"?`} isLoading={remove.isPending} onConfirm={() => deleteTarget && remove.mutate(deleteTarget.id)} onClose={() => setDeleteTarget(null)} />
    </>
  )
}
